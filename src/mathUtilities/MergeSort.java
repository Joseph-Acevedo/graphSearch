...
public class MergeSort 
{
	 ...
	 */
	private static void mergeSort(Node[] a, Node[] tmp, int left, int right)
	{
		if( left < right )
		{
	 ...
	 */
    private static void merge(Node[] a, Node[] tmp, int left, int right, int rightEnd )
    {
        ...
        while(left <= leftEnd && right <= rightEnd)
            if(a[left].getDistance() <= a[right].getDistance())
                tmp[k++] = a[left++];
            else
                tmp[k++] = a[right++];
